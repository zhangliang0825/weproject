...
# # # #     #requests是一个不支持异步的模块，不能使用request爬取
...
# # #         # url,headers,data/params和requests使用一致
# # #         # 设置代理单独写:proxy="http://ip:port"
...
# # #     readCount = json_data.get("readCount")
# # #     popCount = json_data.get("popCount")
# # #     replyCount = json_data.get("replyCount")
# # #     attitude1 = [i.get('count') for i in json_data.get("attitude") if i.get("id") == 'jiayou'][0]
# # #     attitude2 = [i.get('count') for i in json_data.get("attitude") if i.get("id") == 'xiaochusheng'][0]
# # #     attitude3 = [i.get('count') for i in json_data.get("attitude") if i.get("id") == 'shencaozuo'][0]
# # #     attitude4 = [i.get('count') for i in json_data.get("attitude") if i.get("id") == 'suanle'][0]
# # #     print(readCount, replyCount, popCount, attitude1, attitude2, attitude3, attitude4)
# # #
# # #
# # # tasks = []
# # # urls = ['https://mobilegw.alipay.com/mgw.htm?operationType=alipay.secuprod.secucommunity.comment.getH5Comment&requestData=%5B%7B%22commentId%22%3A%222019011062864059bdb501e26a06463dbd4ae7e94b3ab6f8%22%7D%5D&_=1622599907073&callback=1',
# # #         'https://mobilegw.alipay.com/mgw.htm?operationType=alipay.secuprod.secucommunity.comment.getH5Comment&requestData=%5B%7B%22commentId%22%3A%222019011062864059bdb501e26a06463dbd4ae7e94b3ab6f8%22%7D%5D&_=1622599907073&callback=1']
# # # for url in urls:
# # #
...
# #     async with aiofiles.open("蚂蚁财富数据.csv","r",encoding="gb18030") as fp:
# #         # 读取每行
# #         async for line in fp:
# #             print(line.strip())
# # def read():
# #     file = csv.reader(open('蚂蚁财富数据.csv', 'r', encoding='gb18030'))
# #     for i in file:
# #         print(i)
# #
...
# #     loop = asyncio.get_event_loop()
# #     loop.run_until_complete(asyncio.ensure_future(read2_demo()))
# #     # read()
# #     end_time = time.time()
...
#
#
# async def detail_data(title, link, media, str_date, semaphore):
...
#         "SELECT title, REPLACE(SUBSTRING(link,71),'&hasSpace=NO','') as link,media,str_date FROM cx0308 WHERE str_date BETWEEN 20210501 AND 20210515 AND post_type = 10")
#     return r2
#
#     conn.close()
#
...
#         "SELECT  title, REPLACE(SUBSTRING(link,71),'&hasSpace=NO','') as link,media,str_date FROM cx0308 WHERE str_date BETWEEN 20210515 AND 20210531 AND post_type = 10  ")
#
#     return r3
#
#
...
#
# async def main_get():
#     result = await asyncio.gather(data1(), data2())  # 子生成器
...
#     aa_list = [tuple(item) for item in file]
#     for item in aa_list:
...
#         title, str_date, media, readnumCount, popCount, replyCount, attitude1, attitude2, attitude3, attitude4))
#     for key, vlaue in d_dict.items():
#
#         k, j, p, t1, t2, t3, t4 = [[] for _ in range(7)]
#         for info in vlaue:
#             q, w, e, r, t, y, u = info[-7:]
#             k.append(int(q)), j.append(int(w)), p.append(int(e)), t1.append(int(r)),
#             t1.append(int(r)), t2.append(int(t)), t3.append(int(y)), t4.append(int(u))
#         k1, j1, p1, t5, t6, t7, t8 = sum(k), sum(j), sum(p), sum(t1), sum(t2), sum(t3), sum(t4)
#         for info in vlaue:
#             info = info + (k1, j1, p1, t5, t6, t7, t8, link)
#             # with open('蚂蚁财富数据统计数据.csv','a',encoding='gb18030',newline='') as f:
#             #             #     writer = csv.writer(f)
#
...
#     start_time = time.time()
#     loop = asyncio.get_event_loop()
#     result = loop.run_until_complete(main_get())
#     semaphore = asyncio.Semaphore(80)
#     tasks = [asyncio.ensure_future(detail_data(title, link, media, str_date, semaphore))
#              for title, link, media, str_date in result if '号' in media]
...
last_month_start = '2021'+start_month
last_month_end = '2021'+end_month
