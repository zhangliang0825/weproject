...
class ToutiaoSpider(scrapy.Spider):
    ...
    def get_as_cp_signature(self, meida: str, cookie: str, behot_time: int = 0) -> Dict[str, str]:
        ...
        sign = os.popen(
            'node {path1} {url} {cookies}'.format(path1=path1, url='"' + url + '"', cookies='"' + cookie + '"'))
    ...
    def start_requests(self):
        ...
        sql = 'select num from num_crawl where id = 7'
        self.cursor.execute(sql)
        self.num = self.cursor.fetchone()[0]
        self.end_num = self.num + 1
        sql = 'update num_crawl set num = %s where id = 7'
        self.cursor.execute(sql, (self.end_num))
        self.db.commit()
        sql = '''SELECT distinct media,link,token,url,is_jj FROM media WHERE post_type IN (1) AND link <> "" and token is not NULL and id between 35000 and 42000  and TYPE IS NULL'''
        self.cursor.execute(sql)
        ...
        for meida, media_id, token, start_num,is_jj in all_data_list:
            ...
            if int(self.num) - int(start_num) > 1:
                print(meida,media_id,int(self.num), int(start_num), int(self.num) - int(start_num), '数据通过')

                meta = {'media': media_id, 'media_name':meida,'token': token, 'dont_redirect': True,
    ...
    def parse(self, response):
        ...
        if r_data:
            ...
            execute_num = self.cursor.rowcount
            self.db.commit()
            self.mylog.info((media,media_name, execute_num, '数字更新。。'))
            for item in r_data:
